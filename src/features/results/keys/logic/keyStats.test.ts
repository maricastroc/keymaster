...
// Minimal keystroke builder; charIndex is irrelevant to per-key analysis.
function ks(
  expectedChar: string,
  typedChar: string,
  timestampMs: number,
...
describe('analyzeKeys', () => {
  ...
  it('lowercases the expected key so case is a per-key error, not a new key', () => {
    ...
    expect(profile.a.errors).toBe(1); // 'a' !== 'A'
  });

  it('ignores non-letter expected chars (extras, punctuation, spaces)', () => {
    const profile = analyzeKeys([
      ks('a', 'a', 100),
      ks('', 'x', 200), // extra char beyond a word
      ks('1', '1', 300), // digit
    ]);
    expect(Object.keys(profile)).toEqual(['a']);
  });
  ...
  it('measures dwell as the gap from the previous keystroke, capped', () => {
    const profile = analyzeKeys([
      ks('a', 'a', 0),
      ks('b', 'b', 150), // 150ms after a
      ks('c', 'c', 5000), // 4850ms pause — ignored (over cap)
    ]);
    expect(profileToStats(profile).find((s) => s.key === 'b')?.avgMs).toBe(150);
    // 'c' had a dwell over the cap, so no timed sample -> avgMs 0
...
describe('selectWeakKeys', () => {
  ...
  it('ranks error-prone keys ahead of clean ones', () => {
    const weak = selectWeakKeys([
      stat('a', 10, 0, 100), // clean & fast
      stat('b', 10, 5, 120), // very error-prone
      stat('c', 10, 1, 110), // slightly error-prone
    ]);
    expect(weak[0].key).toBe('b');
    expect(weak.map((s) => s.key)).not.toContain('a'); // clean key excluded
  });
  ...
  it('falls back to slowest keys on a flawless, timed run', () => {
    const weak = selectWeakKeys([
      stat('a', 10, 0, 90),
      stat('b', 10, 0, 90),
      stat('c', 10, 0, 300), // no errors anywhere, but clearly slowest
    ]);
